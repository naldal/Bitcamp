...
function oll(){
	// 유효성 체크
	if (document.UpdateForm.name.value == "") // 현재문서.폼이름.태그이름.value
		alert("이름을 입력하세요");
	...
	// else if(document.writeForm.pwd.value=="")
		...
		alert("비밀번호를 입력하세요");
	else if (document.UpdateForm.repwd.value != document.UpdateForm.password.value)
		alert("비밀번호가 맞지 않습니다");
	else
		document.UpdateForm.submit(); // 현재문서.폼이름.액션찾기
}


function checkPost() {
	window.open("checkPost.jsp", "", "width=400 height=300 scrollbars=yes");
}
